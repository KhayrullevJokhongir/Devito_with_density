...
def src_rec(p, model, src_coor, rec_coor, time_range_input):
    src = RickerSource(name='src', grid=model.grid,
                       f0=f0, time_range=time_range_input)
    src.coordinates.data[:] = src_coor

    # Create symbol for receivers
    rec = Receiver(name='rec', grid=model.grid,
                   npoint=rec_coor.shape[0], time_range=time_range_input)
...
def modelling_AWS(model, src_coor, rec_coor, time_range_input, tn, shot_number):
    ...
    src_rec_expr, src, rec = src_rec(p, model, src_coor, rec_coor, time_range_input)

    eqn = create_stencil(model, p, v)

    op = Operator(eqn + src_rec_expr, subs=model.spacing_map)

    op(time=time_range_input.num-1, dt=dt, src=src, rec=rec)

    trimmed_data = p.data[1, nbl:-nbl, nbl: -nbl, nbl: -nbl]

    return trimmed_data, rec, src_coor, rec_coor, shot_number, tn
...
def plot_receiver(rec, src_coor, rec_coor, tn, visualize, save):
    plt.figure(1)
...
def plot_sim_cube(trimmed_data, dx, dy, dz, src_coor, rec_save, tn, visualize, save):
    #Find length of axis
...
simulation_times_storage = [10, 500, 1000, 1500, 2000, 2500, 3000, 3500, 4000, 4500, 5000]
#-------------------------------------------------------------------------------------#
...
for set in range(1):
    with cf.ProcessPoolExecutor(max_workers=CPU_number) as executor:
        for i in range(0,11):
            tn = simulation_times_storage[i]
            time_range = TimeAxis(start=t0, stop=tn, step=dt)
            src_coor = source_coor_storage[0]
            rec_coor = recivers_coor_storage[0] 
            #######################################################################################
            #--------------------------SIMULATION TAKES PLACE HERE--------------------------------#
            #######################################################################################
            simulation = executor.submit(modelling_AWS, model, src_coor, rec_coor, time_range, tn, shot_number=1)
            simulation_storage.append(simulation)
    
        for simulation in cf.as_completed(simulation_storage):
            
            trimmed_data, rec, src_coor_save, rec_coor_save, shot_number, tn = simulation.result()
        
            #######################################################################################
            #--------------------PLOTTING AND SAVING PLOTS TAKE PLACE HERE------------------------#
            #######################################################################################
            # Plot (and save if desired) the simulation result in 3D
            plot_sim_cube(trimmed_data, dx, dy, dz, src_coor_save, rec, tn, visualize=False, save=True)

            #######################################################################################
            # Plot (and save if desired) the receivers recording
            plot_receiver(rec, src_coor_save, rec_coor_save, tn, visualize=False, save=True)
